(function () {
    const modalEl = document.getElementById("mainKakaoMapModal");
    const openBtn = document.getElementById("openMainKakaoMapBtn");
    const closeBtn = document.getElementById("closeMainKakaoMapBtn");
    const titleEl = document.getElementById("mainKakaoMapTitle");
    const resultEl = document.getElementById("mainKakaoMapResult");
    const mapContainer = document.getElementById("mainKakaoMap");
    ...
    function getKnownMapItemCount() {
        if (typeof window.getMainSearchMapItemCount === "function") {
            try {
                const count = Number(window.getMainSearchMapItemCount());
                if (Number.isFinite(count)) return Math.max(0, count);
            } catch (error) {
                console.error("failed to read main map item count", error);
            }
        }
    ...
    function ensureKakaoSdk() {
        ...
        sdkPromise = new Promise((resolve, reject) => {
            const existingScript = document.getElementById("mainKakaoMapSdk");
            if (existingScript) {
    ...
    async function getItemsFromSource(explicitItems, options = {}) {
        if (Array.isArray(explicitItems)) {
            return normalizeItems(explicitItems);
        }
        if (typeof window.getMainSearchMapItems === "function") {
            try {
                const items = await Promise.resolve(window.getMainSearchMapItems(options));
                return normalizeItems(items);
            } catch (error) {
                if (isAbortError(error)) throw error;
                console.error("failed to read main map items", error);
            }
        }
        return normalizeItems(latestItems);
    }
            ...
            ">
                <div style="padding: 11px 12px 12px;">
                    ...
                    ">
                        ...
                        <span style="font-size:10px; font-weight:700; color:#64748b;">상세 보기</span>
                    </div>
                </div>
            </div>
        `;
    ...
    async function renderMarkers(items) {
        ...
        for (const item of normalized) {
            ...
            const hoverPopup = createHoverPopup(position, buildInfoContent(item), item.detail_url);
        ...
        fitBoundsOrCenter();
        if (renderedCount > 0) {
            setResult(`검색결과 ${renderedCount}건 표시 (상세연결 ${linkableCount}건).`);
            return;
        }
        setResult("검색 결과 주소를 좌표로 변환하지 못했습니다.");
    }

    async function refreshMarkers(explicitItems, options = {}) {
        const sourceItems = await getItemsFromSource(explicitItems, options);
        latestItems = normalizeItems(sourceItems);
        if (!mapInitialized || options.skipRender) return true;
        await renderMarkers(latestItems);
        return true;
    }

    async function openMapModal() {
        setMapTitle();
        const knownItemCount = getKnownMapItemCount();
        ...
        try {
            ...
            await refreshMarkers(undefined, {
                skipRender: true,
    ...
    window.addEventListener("main:searchResultsUpdated", () => {
